...
public class OrdersServiceImpl implements OrdersService {
	...
	public void buyResource(Long userId, Long itemId, List<ResourceShop> shop) {
		...
		for (ResourceShop singleShop : shop) {
			...
			// 총 주문 금액 설정
			if (orders.getOrdersPrice() == null) {
				orders.setOrdersPrice(0L); // 호출한 값이 없을수도 있음으로 초기값 설정
			}
		...
		for (OrdersDetails ordersDetails : ordersDetailsList) {
			...
			log.info("주문 상세 아이템번호 세팅 확인: {}", ordersDetails.getItemId(), "주문상세 주문번호 세팅 확인: {}",
					ordersDetails.getOrdersId());
